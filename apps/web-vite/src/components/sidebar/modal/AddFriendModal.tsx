...
// import type { FriendRequestsType, ReceivedFriendRequest } from '@/types'

export default function AddFriendModal() {
    const [isAddFriendModalOpen, setIsAddFriendModalOpen] = useState(false)
    const [searchQuery, setSearchQuery] = useState<string>('')
    const [error, setError] = useState<string | undefined>()
    const [friendRequests, setFriendRequests] = useState<any>({ receivedRequests: [], sentRequests: [] })

    const handleUpdateReceivedRequests = (updatedRequests: any[]) => {
        setFriendRequests({
            ...friendRequests,
    ...
    useEffect(() => {
        const fetchRequests = async () => {
            try {
                const res: any = [];//await fetchFriendRequests()
