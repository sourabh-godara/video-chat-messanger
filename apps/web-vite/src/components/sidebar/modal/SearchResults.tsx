...
// import type { FriendRequestsType, User } from '@/types';
...
export default function SearchResultCard({ users, friendRequests }: Props) {
    const [sentFriendRequests, setSentFriendRequests] = useState(new Set(friendRequests.sentRequests.map((req: any) => req.receiverId)));
    const [receivedFriendRequests] = useState(new Set(friendRequests.receivedRequests.map((req: any) => req.senderId)));
